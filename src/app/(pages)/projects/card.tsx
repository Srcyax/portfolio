...
export default function CardProject({image, title, desc, technologies, github, site} : {image : string, title : string, desc : string, technologies : string[], github : string, site : string}){
    ...
    return (
        <div className="m-12 border-2 border-indigo-600 rounded-lg border-purple hover:shadow-neon opacity-50 hover:opacity-100 w-60 h-auto p-5 flex flex-col items-center transition-all duration-200 ease"> 
            {!imageLoaded.projectImage ? <Skeleton className='w-40 h-40 rounded-full'/> : null}
            <LazyLoadImage className={imageLoaded.projectImage ? 'w-40 h-40 rounded-full' : ''} src={image} beforeLoad={() => handleImageLoad('projectImage')}/>
                    ...
                    <div key={index}>
                        ...
                        technologiesUrl.filter(t => t[0] === tech).map((filteredTech, index) => (
                            <div key={index} className="group grid place-items-center">
                                {!imageLoaded.techImage ? <Skeleton className='w-6 h-6 rounded-full'/> : null}
                                <LazyLoadImage className={imageLoaded.techImage ? 'opacity-70 hover:opacity-100 hover:w-5 transition-all duration-200 ease' : ''} width={22} src={filteredTech[1]} beforeLoad={() => handleImageLoad('techImage')}/>
